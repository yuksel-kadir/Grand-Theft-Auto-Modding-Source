...
//MESSAGES
const std::string TextUtils::STAUNTON_MSG = "When the Callahan Bridge is repaired or the tunnel constructions is finished you will be able to go to Staunton Island.";
const std::string TextUtils::SHORESIDE_MSG = "When the Shoreside Lift Bridge is repaired or the tunnel constructions is finished you will be able to go to Shoreside Vale.";
...
TextInformation TextUtils::destinationTextInfo = TextInformation("", TextUtils::COLOR_ORANGE, 0, 0, 0, eTextPosition::ON_RADAR, false);
...
void TextUtils::PreviousDestination(int8_t& destinationIndex) {
    if (destinationIndex == 0) {
        bool shoresideValeCheck = TaxiUtils::CheckBridgeStatus(eLevelName::SHORESIDE_VALE);
        if (shoresideValeCheck) {
            destinationIndex -= 1;
        }
        else {
            bool stauntonCheck = TaxiUtils::CheckBridgeStatus(eLevelName::STAUNTON_ISLAND);
            if (stauntonCheck){
                destinationIndex = 19;
            }
            else {
...
void TextUtils::PrintBridgeHelpMessage(eLevelName levelName) {
    switch (levelName)
    {
    case STAUNTON_ISLAND:
        TextUtils::PrintQuickHelpMessage(STAUNTON_MSG);
        break;
    case SHORESIDE_VALE:
        TextUtils::PrintQuickHelpMessage(SHORESIDE_MSG);
        break;
    default:
        break;
    }
