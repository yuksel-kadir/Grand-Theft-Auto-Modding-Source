...
switch (HOST) {
  case "vc_unreal":
    ...
    break;
  case "sa_unreal":
    log("GTA SA IS NOT SUPPORTED BY THE SCRIPT: " + HOST);
    break;
  case "gta3_unreal":
    log("GTA 3 NOT SUPPORTED BY THE SCRIPT: " + HOST);
    break;
  case "vc":
    log("THE GAME IS: " + HOST);
    loadInitialValues();
    break;
  default:
    log("THIS GAME IS NOT SUPPORTED BY THE SCRIPT: " + HOST);
    break;
}

/**
 * Loads initial variable values.
 */
function loadInitialValues(){
  taxiModelIds = [150, 168, 216];
  taxiDriverModelID = 74;
  isGameIsSupported = true;
  log("Initial values are loaded.");
}
/**
 * Loads taxi model.
 */
